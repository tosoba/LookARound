...
fun ShimmerAnimation(modifier: Modifier = Modifier) {
    /**
     * Create InfiniteTransition which holds child animation like [Transition] animations start
     * running as soon as they enter the composition and do not stop unless they are removed
     */
    ...
    /**
     * Create a gradient using the list of colors Use Linear Gradient for animating in any direction
     * according to requirement start=specifies the position to start with in cartesian like system
     * ```
...
fun ShimmerItem(brush: Brush, modifier: Modifier = Modifier) {
    Spacer(modifier = Modifier.background(brush = brush) then modifier)
}
